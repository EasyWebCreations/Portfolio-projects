...
class Friend{
    ...
    public function am_i_the_req_sender($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `request` WHERE 	sender = ? AND receiver = ?";
            $stmt = $this->db->prepare($sql);
    ...
    public function is_request_already_sent($my_id, $user_id){
        ...
        try{
            $sql = "SELECT * FROM `request` WHERE (sender = :my_id AND receiver = :frnd_id) OR (sender = :frnd_id AND receiver = :my_id)";
    ...
    public function make_pending_friends($my_id, $user_id){
        ...
        try{
            $sql = "INSERT INTO `request`(sender, receiver) VALUES(?,?)";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$my_id, $user_id]);
    ...
    public function cancel_or_ignore_friend_request($my_id, $user_id){
        ...
        try{
            $sql = "DELETE FROM `request` WHERE (sender = :my_id AND receiver = :frnd_id) OR (sender = :frnd_id AND receiver = :my_id)";
    ...
    public function make_friends($my_id, $user_id){
        ...
        try{
            ...
            $delete_pending_friends = "DELETE FROM `request` WHERE (sender = :my_id AND receiver = :frnd_id) OR (sender = :frnd_id AND receiver = :my_id)";
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, fname, lname FROM `request` JOIN per_details ON request.sender = per_details.id WHERE receiver = ?";
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT * FROM `per_details` WHERE id = ?";
