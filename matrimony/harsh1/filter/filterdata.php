...
while ($row1 = mysqli_fetch_assoc($query)) {
  ...
  //$result = mysqli_query($conn, "SELECT * FROM per_details WHERE id = {$sender}");
  //$row12 = mysqli_fetch_assoc($result);
  //$stmt = $conn->prepare("SELECT * FROM per_details WHERE gender<>?");
  //$stmt->bind_param("s", $row12['gender']);
  //$stmt->execute();
  //$result1 = $stmt->get_result();
  // $row1 = $result->fetch_assoc();
  // while ($row1 = mysqli_fetch_assoc($result)) {//
  //while ($row12 = mysqli_fetch_assoc($result)) {
  $query2 = "SELECT * from likes where liked_by={$sender} and liked_to={$row1['id']}";
  ...
  $query3 = "SELECT * from dislikes where disliked_by={$sender} and disliked_to={$row1['id']}";
  ...
  $queryreport = "SELECT * from report where reported_by={$row['id']} and reported_to={$row1['id']}";
    ...
    $output .= '<div class="card">
                  ...
                  <div class="cardTextOut">
                    ...
                    <form class="cardMoreOut" method="post" target="frame">
                      ...
                      <input name="liked_by" value="' . $row['id'] . '" style="display:none"></input>
