...
$output = '';
if (is_numeric($searchPeople)) {
  // echo 'Numeric';
  $sql = "SELECT fname,lname,profile_img,occupation,height,age,id FROM per_details WHERE id <> '11111' AND id LIKE '{$searchPeople}%'";
  ...
  if (mysqli_num_rows($query) > 0) {
    ...
    $output = 'Please Enter Valid Name Or UserID To Search...';
  }
} else if (ctype_alpha($searchPeople)) {
  // echo 'Alphabetic';
  $sql = "SELECT fname,lname,profile_img,occupation,height,age,id FROM per_details WHERE id <> '11111' AND fname LIKE '{$searchPeople}%' OR lname LIKE '{$searchPeople}%'";
  $query = mysqli_query($conn, $sql);
  if (mysqli_num_rows($query) > 0) {
    while ($row2 = mysqli_fetch_assoc($query)) {
      // $sql1 = "SELECT * FROM per_details WHERE id={$row1['liked_by']}";
  ...
  // echo 'Alphanumeric';
  // echo 'Please Enter Valid Name Or UserID To Search...';
  $output = 'Please Enter Valid Name Or UserID To Search...';
}
echo $output;
